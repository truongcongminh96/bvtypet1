        ...
        <MotionGroup className="mt-14 grid gap-12 lg:grid-cols-12 lg:gap-6" amount={0.1} stagger={0.12}>
          <MotionItem className="lg:col-span-8" direction="left">
            ...
            >
              ...
              <div className="relative z-10 -mt-8 ml-4 max-w-[44rem] bg-background px-4 pt-5 sm:-mt-14 sm:ml-10 sm:px-7 lg:-mr-24 lg:ml-16">
          ...
          {remaining.length > 0 ? (
            <MotionGroup className="content-start lg:col-span-4 lg:mt-28 lg:pl-8" stagger={0.09}>
              ...
              {remaining.map((article, index) => {
                ...
                return (
                  <MotionItem
                    key={article.slug}
                    className={index === 1 ? "lg:ml-6" : index === 2 ? "lg:ml-2" : undefined}
                    direction="right"
                    mobileDirection="left"
                  >
                    <Link
                      href={`/cam-nang/${article.slug}`}
                      className="group grid grid-cols-[5.5rem_1fr] gap-4 border-b border-border py-6 focus-visible:outline focus-visible:outline-3 focus-visible:outline-offset-3 focus-visible:outline-brand-blue-dark"
                    >
                      <figure className="relative aspect-[4/3] overflow-hidden rounded-[0.75rem_1.75rem_0.75rem_0.75rem] bg-surface-soft">
                        <Image
                          src={image.src}
                          alt={image.alt}
                          fill
                      ...
                      <div className="min-w-0">
                        <p className="text-[0.68rem] font-semibold text-brand-blue-dark">
                          {guideCategoryLabels[article.category]}
                        </p>
                        <h3 className="mt-2 font-display text-xl font-semibold leading-[1.1] text-text-primary">
                          {article.title}
                        </h3>
